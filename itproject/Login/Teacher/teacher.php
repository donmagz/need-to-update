...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] == 'cancel') {
        $sql = "UPDATE appointmentdb SET Status = 'Cancelled' WHERE Student_ID = ? AND teacher_name = ?";
        $statement = $conn->prepare($sql);
        $statement->bind_param("is", $studentId, $teacherName);
        if ($statement->execute()) {
            header("Location: teacher.php");
            ...
            <tbody>
                <?php foreach ($appointments['Pending'] as $row): ?>
                    <tr>
                        ...
                        <td>
                            <a href="teacher.php?action=accept&id=<?= $row['Student_ID'] ?>" class="btn btn-success btn-sm">Accept</a>
                            <a href="teacher.php?action=cancel&id=<?= $row['Student_ID'] ?>" class="btn btn-danger btn-sm">Cancel</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
            ...
            <tbody>
                <?php foreach ($appointments['Ongoing'] as $row): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['Description']) ?></td>
                        <td>
                            <a href="teacher.php?action=complete&id=<?= $row['Student_ID'] ?>" class="btn btn-primary btn-sm">Complete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
